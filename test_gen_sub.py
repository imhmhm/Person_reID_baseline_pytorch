...
if opt.multi:
    image_datasets = {x: datasets.ImageFolder( os.path.join(data_dir,x) ,data_transforms) for x in ['gallery','query','gen_query']}
    dataloaders = {x: torch.utils.data.DataLoader(image_datasets[x], batch_size=opt.batchsize,
                                             shuffle=False, num_workers=16) for x in ['gallery','query','gen_query']}
else:
...
if opt.multi:
    mquery_path = image_datasets['gen_query'].imgs
...
with torch.no_grad():
    ...
    if opt.multi:
        mquery_feature = extract_feature(model,dataloaders['gen_query'])
