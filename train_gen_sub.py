...
name = opt.name
assert(opt.num_per_id == (opt.prop_real + opt.prop_gen))
proportion = [opt.prop_real, opt.prop_gen]
...
class GenSampler(Sampler):
    ...
    def __init__(self, data_source, batch_size, num_instances, proportion):
        ...
        self.real = proportion[0]
        self.gen = proportion[1]
        ...
        self.pids = list(self.index_dic_real.keys())
        ...
        for pid in self.pids:
            ...
            for idx in idxs_real:
                ...
                if len(batch_idxs_real) == self.real:
                    ...
                    batch_idxs_real = []
            for idx in idxs_gen:
                batch_idxs_gen.append(idx)
                if len(batch_idxs_gen) == self.gen:
                    batch_idxs_dict_gen[pid].append(batch_idxs_gen)
                    batch_idxs_gen = []
...
real_datasets = genDataset(os.path.join(data_dir, 'train' + train_all),
                           data_transforms['train'], flag=0)
        ...
        for phase in ['train', 'val']:
            ...
            for data in tqdm(dataloaders[phase]):
                ...
                if not opt.PCB:
                    ...
                    loss = criterion(part[0], labels, flags)
                    for i in range(num_part-1):
                        loss += criterion(part[i+1], labels, flags)
