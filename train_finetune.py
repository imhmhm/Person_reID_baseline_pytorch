...
image_datasets['train'] = datasets.ImageFolder(os.path.join(data_dir, 'gen_train_7p_v4_x1'),
                                               data_transforms['train'])
...
if opt.use_dense:
    model_structure = ft_net_dense(len(class_names), opt.droprate)
elif opt.use_resnext:
    model_structure = resnext50(751)
else:
    model_structure = ft_net(len(class_names), opt.droprate)

if opt.PCB:
    model_structure = PCB(len(class_names))
